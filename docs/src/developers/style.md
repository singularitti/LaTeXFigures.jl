...
!!! info
    The style guide is always a work in progress, and not all LaTeXFigures code
    follows the rules. When modifying LaTeXFigures, please fix the style violations
...
`LaTeXFigures` uses [`JuliaFormatter`](https://github.com/domluna/JuliaFormatter.jl) as
an auto-formatting tool.

We use the options contained in [`.JuliaFormatter.toml`](https://github.com/singularitti/LaTeXFigures.jl/blob/main/.JuliaFormatter.toml).

To format your code, `cd` to the `LaTeXFigures` directory, then run:
...
!!! info
    A continuous integration check verifies that all PRs made to `LaTeXFigures` have

...
[`LaTeXFigures`](https://github.com/singularitti/LaTeXFigures.jl).
...
2. Run the following commands and wait for them to finish:
   ...
   julia> Pkg.add("LaTeXFigures")
   ```

3. Run

   ```julia-repl
   julia> using LaTeXFigures
   ```
...
version of `LaTeXFigures`, type

```@repl
using Pkg
Pkg.update()
pkg"add https://github.com/singularitti/LaTeXFigures.jl"
```
...
our [GitHub repository](https://github.com/singularitti/LaTeXFigures.jl)
for new releases.
Once we release a new version, you can update `LaTeXFigures` by typing

```@repl
using Pkg
Pkg.update("LaTeXFigures")
Pkg.gc()
```

in the Julia REPL.

## Uninstall and reinstall `LaTeXFigures`
